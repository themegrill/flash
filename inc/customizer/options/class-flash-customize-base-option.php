...
/**
 * Class Flash_Customize_Base_Option
 */
class Flash_Customize_Base_Option {
	...
	/**
	 * Flash_Customize_Base_Option constructor.
	 */
	public function __construct() {

		// Register customizer options.
		add_action( 'customize_register', array( $this, 'flash_customizer_options' ) );
		// Generate styles from customizer inputs.
		add_action( 'wp_enqueue_scripts', array( $this, 'flash_enqueue_style' ) );
	...
	protected function get_menu_options() {
		...
		$menu_options         = array();
		$menu_options['none'] = esc_html__( 'None', 'flash' );
	 ...
	 */
	public function flash_customizer_options( $wp_customize ) {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( ! $is_custom_control ) {
				...
				$control_type    = 'Flash_Customize_' . $control_type_uc . '_Control';
	 ...
	 */
	public function flash_enqueue_style() {

		$output = $this->generate_style();

		wp_add_inline_style( 'flash-style', $output );
	}
