...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.btn-get-started' ).click( function ( e ) {
		...
		// Change button text.
		jQuery( this ).text( flash_redirect_demo_page.btn_text );
		...
		var data = {
			...
			security : flash_redirect_demo_page.nonce,
			state    : state
		};
		...
		$.ajax( {
			...
			success : function ( response ) {
				extra_uri = '';
				if ( jQuery( '.flash-message-close' ).length ) {
					dismiss_nonce = jQuery( '.flash-message-close' ).attr( 'href' ).split( '_flash_notice_nonce=' )[1];
					extra_uri     = '&_flash_notice_nonce=' + dismiss_nonce;
				}
